...
const routeRef = { params: {}, path: '/monitoring/dashboard' };
...
vi.mock('@/hub/shared/composables/useSidebar', async () => ({
  useSidebar: () => ({
    get collapsed() {
      return collapsedRef;
    },
...
describe('HubSidebar', () => {
  beforeEach(() => {
    ...
    routeRef.path = '/monitoring/dashboard';
  ...
  it('renders Monitoring section headings when not collapsed', () => {
    const wrapper = mountSidebar();
  ...
  it('shows Hub sections on the hub home', () => {
    routeRef.path = '/';
    const wrapper = mountSidebar();
    expect(wrapper.text()).toContain('nav.sections.account');
    expect(wrapper.text()).not.toContain('nav.sections.catalog');
  });
});
