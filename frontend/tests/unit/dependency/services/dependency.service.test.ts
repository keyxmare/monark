...
describe('dependencyService', () => {
  ...
  it('list calls GET /dependency/dependencies with default params', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/monitoring/dependency/dependencies?page=1&per_page=20');
  });

  it('list calls GET with custom page and perPage', async () => {
    vi.mocked(api.get).mockResolvedValue({ data: { items: [] }, status: 200 });
    await dependencyService.list(2, 50);
    expect(api.get).toHaveBeenCalledWith('/monitoring/dependency/dependencies?page=2&per_page=50');
  });
  ...
  it('list calls GET with projectId filter', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith(
      '/monitoring/dependency/dependencies?page=1&per_page=20&project_id=proj-1',
    );
  });
  ...
  it('get calls GET /dependency/dependencies/:id', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/monitoring/dependency/dependencies/dep-1');
  });
  ...
  it('create calls POST /dependency/dependencies', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/monitoring/dependency/dependencies', input);
  });
  ...
  it('update calls PUT /dependency/dependencies/:id', async () => {
    ...
    expect(api.put).toHaveBeenCalledWith('/monitoring/dependency/dependencies/dep-1', input);
  });

  it('remove calls DELETE /dependency/dependencies/:id', async () => {
    vi.mocked(api.delete).mockResolvedValue(undefined);
    await dependencyService.remove('dep-1');
    expect(api.delete).toHaveBeenCalledWith('/monitoring/dependency/dependencies/dep-1');
  });
  ...
  it('sync calls POST /dependency/sync', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/monitoring/dependency/sync', {});
  });

  it('stats calls GET /dependency/stats without params', async () => {
    vi.mocked(api.get).mockResolvedValue({ data: {}, status: 200 });
    await dependencyService.stats();
    expect(api.get).toHaveBeenCalledWith('/monitoring/dependency/stats');
  });
  ...
  it('stats calls GET /dependency/stats with all params', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith(
      '/monitoring/dependency/stats?project_id=p1&package_manager=npm&type=runtime',
    );
  });
  ...
  it('stats calls GET /dependency/stats with partial params', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/monitoring/dependency/stats?package_manager=composer');
  });
});
