...
describe('projectService', () => {
  ...
  it('list calls GET /catalog/projects with default pagination', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/monitoring/catalog/projects?page=1&per_page=20');
  });

  it('list calls GET /catalog/projects with custom pagination', async () => {
    vi.mocked(api.get).mockResolvedValue({ data: { items: [], total: 0 }, status: 200 });
    await projectService.list(2, 10);
    expect(api.get).toHaveBeenCalledWith('/monitoring/catalog/projects?page=2&per_page=10');
  });
  ...
  it('get calls GET /catalog/projects/:id', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/monitoring/catalog/projects/abc');
  });
  ...
  it('create calls POST /catalog/projects', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/monitoring/catalog/projects', input);
  });
  ...
  it('update calls PUT /catalog/projects/:id', async () => {
    ...
    expect(api.put).toHaveBeenCalledWith('/monitoring/catalog/projects/abc', input);
  });

  it('remove calls DELETE /catalog/projects/:id', async () => {
    vi.mocked(api.delete).mockResolvedValue(undefined);
    await projectService.remove('abc');
    expect(api.delete).toHaveBeenCalledWith('/monitoring/catalog/projects/abc');
  });
  ...
  it('scan calls POST /catalog/projects/:id/scan', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/monitoring/catalog/projects/abc/scan', {});
  });
});
