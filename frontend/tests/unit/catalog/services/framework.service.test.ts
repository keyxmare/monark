...
describe('frameworkService', () => {
  it('list builds a paginated URL', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/monitoring/catalog/frameworks?page=3&per_page=50');
  });
  ...
  it('list appends project_id when provided', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith(
      '/monitoring/catalog/frameworks?page=1&per_page=20&project_id=proj-42',
    );
  });
  ...
  it('get uses crud fetch by id', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/monitoring/catalog/frameworks/fw-1');
  ...
  it('remove deletes by id', async () => {
    ...
    expect(api.delete).toHaveBeenCalledWith('/monitoring/catalog/frameworks/fw-1');
  });
});
