...
const BASE_URL = '/monitoring/catalog/providers';
...
export const providerService = {
  ...
  syncAllGlobal(force = false): Promise<ApiResponse<SyncJobResponse>> {
    const params = force ? '?force=1' : '';
    return api.post<ApiResponse<SyncJobResponse>>(`/monitoring/catalog/sync-all${params}`, {});
  },

  getSyncJob(id: string): Promise<ApiResponse<SyncJobProgress>> {
    return api.get<ApiResponse<SyncJobProgress>>(`/monitoring/catalog/sync-jobs/${id}`);
  },
};
