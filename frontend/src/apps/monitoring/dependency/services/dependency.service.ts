...
export const dependencyService = {
  ...
  sync(): Promise<ApiResponse<{ syncId: string; total: number }>> {
    return api.post<ApiResponse<{ syncId: string; total: number }>>(
      '/monitoring/dependency/sync',
      {},
    );
  },
  ...
  > {
    ...
    >(`/monitoring/dependency/stats${query ? `?${query}` : ''}`);
  },
};
