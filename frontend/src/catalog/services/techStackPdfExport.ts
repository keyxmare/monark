...
export function exportTechStacksPdf(
  ...
  doc.text(`Rapport Stacks Techniques — ${new Date().toLocaleDateString('fr-FR')}`, pageWidth - 15, y, { align: 'right' })
  y += 10
  ...
  if (health) {
    ...
    doc.setFontSize(12)
    doc.setTextColor(...COLORS.dark)
    doc.text(`Score de santé : ${health.percent}% à jour  (${health.active}/${health.total})`, 15, y)
    y += 6
  ...
  if (providers.length > 0) {
    const contentWidth = pageWidth - 30
    const cardWidth = providers.length === 1 ? contentWidth : Math.min((contentWidth - (providers.length - 1) * 5) / providers.length, 130)
    ...
    for (const prov of providers) {
      ...
      doc.setFont('helvetica', 'normal')
      doc.setTextColor(...COLORS.muted)
      doc.text(`${prov.type} — ${prov.projectCount} projet(s)`, cardX + 4 + doc.getTextWidth(prov.name) + 3, innerY)
      innerY += 5
  ...
  const head = [['Projet', 'Langage', 'Framework', 'Version', 'Dernière LTS', 'Écart', 'Statut', 'Release']]
  ...
  const body = sortedRows.map((r, i) => {
    ...
    return [showProject ? r.project : '', r.language, r.framework, r.version, r.latestLts, r.ltsGap, r.status, r.releaseDate]
  })
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(`Généré le ${new Date().toLocaleString('fr-FR')}`, 15, doc.internal.pageSize.getHeight() - 8)
  }
