...
const rule = {
    meta: {
        type: "problem",
        docs: {
            url: (0, utils_1.makeDocsURL)("no-aria-hidden-on-focusable")
        },
        messages: {
    ...
    create(context) {
        return (0, utils_1.defineTemplateBodyVisitor)(context, {
            VElement(node) {
                const hasAriaHidden = (0, utils_1.getElementAttributeValue)(node, "aria-hidden");
                if (hasAriaHidden && (0, hasFocusableElement_1.default)(node)) {
                    context.report({
                        node,
                        messageId: "default"
                    });
