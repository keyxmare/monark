...
use App\Monitoring\Catalog\Domain\Event\FrameworkVersionStatusUpdated;
...
function createFwEmptyProductRepo(): ProductRepositoryInterface
{
    return new class () implements ProductRepositoryInterface {
        public function findByNameAndManager(string $name, ?\App\Hub\Shared\Domain\ValueObject\PackageManager $pm): ?Product
        {
            return null;
        }
        public function findAll(): array
...
describe('FrameworkVersionStatusUpdater', function () {
    ...
    it('saves and dispatches event after refreshing a known framework with versions', function () {
        ...
        $productRepo = new class () implements ProductRepositoryInterface {
            public function findByNameAndManager(string $name, ?\App\Hub\Shared\Domain\ValueObject\PackageManager $pm): ?Product
            {
                return Product::create($name, \App\Monitoring\VersionRegistry\Domain\Model\ProductType::Framework, \App\Monitoring\VersionRegistry\Domain\Model\ResolverSource::EndOfLife, null);
            }
            public function findAll(): array
            {
                return [];
