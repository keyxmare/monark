...
use App\Monitoring\Catalog\Application\CommandHandler\SyncProjectMetadataHandler;
...
describe('SyncProjectMetadataHandler', function () {
    it('updates metadata when remote differs', function () {
        ...
        $metadataEvents = \array_filter($eventBus->dispatched, static fn ($e) => $e instanceof ProjectMetadataSyncedEvent);
        expect($metadataEvents)->toHaveCount(1);
        expect(\array_values($metadataEvents)[0]->changedFields)->toContain('name', 'description', 'visibility');
    });

    it('refreshes the activity cache even when metadata is unchanged', function () {
        $provider = ProviderFactory::create();
        $project = \createMetadataProject($provider);
        ...
        expect($projectRepo->saved)->toBe($project);
        $metadataEvents = \array_filter($eventBus->dispatched, static fn ($e) => $e instanceof ProjectMetadataSyncedEvent);
        expect($metadataEvents)->toBeEmpty();
        $activityEvents = \array_filter($eventBus->dispatched, static fn ($e) => $e instanceof ProjectActivityCacheRefreshed);
    ...
    it('only lists actually changed fields', function () {
        ...
        $metadataEvent = \array_values(\array_filter(
            $eventBus->dispatched,
            static fn ($e) => $e instanceof ProjectMetadataSyncedEvent,
        ))[0] ?? null;
        expect($metadataEvent)->not->toBeNull();
        expect($metadataEvent->changedFields)->toBe(['name']);
        expect($project->getName())->toBe('Renamed Project');
