...
describe('GlobalSyncJob', function (): void {
    ...
    it('starts a step with correct total', function (): void {
        $job = GlobalSyncJob::create();
        $job->startStep(GlobalSyncStep::SyncDependencies, 42);

        expect($job->getCurrentStep())->toBe(3)
            ->and($job->getCurrentStepName())->toBe('sync_dependencies')
    ...
    it('returns completed step names', function (): void {
        $job = GlobalSyncJob::create();
        $job->startStep(GlobalSyncStep::SyncDependencies, 10);
    ...
    it('returns all steps completed when done', function (): void {
        ...
        expect($job->getCompletedStepNames())->toBe(['sync_projects', 'sync_coverage', 'sync_dependencies', 'sync_frameworks', 'scan_cve']);
    });
});
