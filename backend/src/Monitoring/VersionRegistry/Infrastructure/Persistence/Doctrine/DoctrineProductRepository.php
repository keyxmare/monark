...
namespace App\Monitoring\VersionRegistry\Infrastructure\Persistence\Doctrine;
...
final readonly class DoctrineProductRepository implements ProductRepositoryInterface
{
    ...
    public function findByResolverSource(ResolverSource $source): array
    {
        /** @var list<Product> */
        return $this->em->createQueryBuilder()
            ->select('p')
            ->from(Product::class, 'p')
            ->where('p.resolverSource = :source')
