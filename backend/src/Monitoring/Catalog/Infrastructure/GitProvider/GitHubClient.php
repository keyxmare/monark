...
final readonly class GitHubClient implements GitProviderInterface
{
    ...
    {
        $url = \sprintf('%s/repos/%s/languages', $this->baseUrl($provider), $externalProjectId);
    ...
    {
        $url = \sprintf('%s/repos/%s/commits', $this->baseUrl($provider), $externalProjectId);
        ...
        for ($page = 1; $page <= $maxPages; ++$page) {
            try {
                ...
                /** @var list<array{sha: string, commit?: array{author?: array{date?: string}}}> $batch */
            ...
            foreach ($batch as $raw) {
                $dateRaw = $raw['commit']['author']['date'] ?? null;
                if ($dateRaw === null) {
                    continue;
                }
                try {
                ...
                $commits[] = new RemoteCommit($raw['sha'], $authoredAt);
            }
