...
namespace App\Monitoring\Catalog\Infrastructure\GitProvider;
...
final readonly class GitLabClient implements GitProviderInterface
{
    ...
    {
        $url = \sprintf('%s/api/v4/projects/%s/languages', $provider->getUrl(), $externalProjectId);

        try {
            $response = $this->httpClient->request('GET', $url, [
                'headers' => ['PRIVATE-TOKEN' => $provider->getApiToken()],
            ]);
            /** @var array<string, float|int> $data */
            $data = $response->toArray();

            $palette = [];
            foreach ($data as $language => $percent) {
                $palette[$language] = (int) \round(((float) $percent) * 1000);
            }
            \arsort($palette);

            return $palette;
        } catch (Throwable $e) {
            $this->logger->warning('GitLab getRepositoryLanguages failed.', ['error' => $e->getMessage()]);
    ...
    public function getRecentCommits(Provider $provider, string $externalProjectId, DateTimeImmutable $since): array
    {
        ...
        $isoSince = $since->setTimezone(new DateTimeZone('UTC'))->format('Y-m-d\TH:i:s\Z');
        ...
        for ($page = 1; $page <= $maxPages; ++$page) {
            try {
                $response = $this->httpClient->request('GET', $url, [
                    'headers' => ['PRIVATE-TOKEN' => $provider->getApiToken()],
                    'query' => ['since' => $isoSince, 'per_page' => 100, 'page' => $page],
                ]);

                /** @var list<array{id: string, authored_date?: string, committed_date?: string}> $batch */
                $batch = $response->toArray();
            } catch (Throwable $e) {
                $this->logger->warning('GitLab getRecentCommits failed.', ['page' => $page, 'error' => $e->getMessage()]);
                break;
            }
            ...
            foreach ($batch as $raw) {
                $dateRaw = $raw['authored_date'] ?? $raw['committed_date'] ?? null;
                if ($dateRaw === null) {
                    continue;
                }
                try {
                    $authoredAt = new DateTimeImmutable($dateRaw);
                } catch (Throwable) {
                    continue;
                }
                $commits[] = new RemoteCommit($raw['id'], $authoredAt);
            }
