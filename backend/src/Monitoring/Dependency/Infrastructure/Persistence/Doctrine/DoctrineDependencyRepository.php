...
namespace App\Monitoring\Dependency\Infrastructure\Persistence\Doctrine;
...
final readonly class DoctrineDependencyRepository implements DependencyRepositoryInterface
{
    ...
    public function countByProjects(array $projectIds): array
    {
        ...
        $sql = <<<'SQL'
            SELECT project_id::text AS project_id, COUNT(*) AS c
            FROM dependencies
            WHERE project_id::text IN (:ids)
            GROUP BY project_id
            SQL;

        /** @var list<array{project_id: string, c: int|string}> $rows */
        $rows = $conn->fetchAllAssociative($sql, ['ids' => $ids], ['ids' => ArrayParameterType::STRING]);
        $out = [];
        foreach ($rows as $row) {
            $out[$row['project_id']] = (int) $row['c'];
        }
    ...
    public function countVulnerabilitiesByProjects(array $projectIds): array
    {
        ...
        $conn = $this->entityManager->getConnection();
        $sql = <<<'SQL'
            SELECT d.project_id::text AS project_id, COUNT(v.id) AS c
            FROM dependencies d
            INNER JOIN vulnerabilities v ON v.dependency_id = d.id
            WHERE d.project_id::text IN (:ids)
    ...
    public function countVulnerabilitiesBySeverityForProjects(array $projectIds): array
    {
        ...
        $conn = $this->entityManager->getConnection();
        $sql = <<<'SQL'
            SELECT d.project_id::text AS project_id, v.severity AS severity, COUNT(v.id) AS c
            FROM dependencies d
            INNER JOIN vulnerabilities v ON v.dependency_id = d.id
            WHERE d.project_id::text IN (:ids)
            GROUP BY d.project_id, v.severity
            SQL;

        /** @var list<array{project_id: string, severity: string, c: int|string}> $rows */
        $rows = $conn->fetchAllAssociative($sql, ['ids' => $ids], ['ids' => ArrayParameterType::STRING]);
        $out = [];
        foreach ($rows as $row) {
            $pid = $row['project_id'];
            $out[$pid] ??= ['critical' => 0, 'high' => 0, 'medium' => 0, 'low' => 0];
            $severity = $row['severity'];
            if (\in_array($severity, ['critical', 'high', 'medium', 'low'], true)) {
                $out[$pid][$severity] = (int) $row['c'];
            }
        }
    ...
    {
        /** @var list<array{name: string, packageManager: \App\Hub\Shared\Domain\ValueObject\PackageManager}> */
        return $this->entityManager->getRepository(Dependency::class)
            ->createQueryBuilder('d')
    ...
    public function getStats(array $filters = []): array
    {
        ...
        $qbVulns = $this->entityManager->createQueryBuilder()
            ->select('COUNT(v.id)')
            ->from('App\Monitoring\Dependency\Domain\Model\Vulnerability', 'v')
            ->join('App\Monitoring\Dependency\Domain\Model\Dependency', 'd', 'WITH', 'v.dependency = d.id');
